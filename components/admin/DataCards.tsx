...
const DataCards = async ({ fetchWhat }: { fetchWhat: string }) => {
    ...
    return (
        <div className="mb-10 sm:mb-0 mt-5 grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            <Link href="/dashboard/create-post" className="group bg-gray-900/30 py-20 px-4 flex flex-col space-y-2 items-center cursor-pointer rounded-md hover:bg-gray-900/40 hover:smooth-hover">
                ...
                <div className="text-white/50 group-hover:text-white group-hover:smooth-hover text-center">Create post</div>
            </Link>

            {dataSet.map((data) => (
                <div className="relative group bg-white shadow-lg transition-all py-10 sm:py-20 px-4 flex flex-col space-y-2 items-center cursor-pointer rounded-md hover:bg-slate-100 hover:smooth-hover">
                    {data.image &&
                        <Image
